...
- **Bidón automático en 20 L**: cada envase de 20 L de una línea líquida agrega un `BIDON` ($25) al pedido/cotización desde `lib/ops/bidon.ts` (no lo decide el modelo); el resumen lo lista y avisa que **si el cliente entrega los vacíos a cambio, el chofer no se los cobra** (el ajuste es en la entrega, nunca descuento). Excluye artículos de plástico/jarciería de "20 LITROS" (cesto, cubeta, palangana) y **no** aplica a 10 L (ahí se pregunta). Cotización→pedido no duplica la línea. Cobertura: `lib/ops/bidon.test.ts` + evals `pedidos/bidon-20l-automatico` y `pedidos/bidon-a-cambio`.

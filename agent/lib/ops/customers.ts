...
import { getAgentTeamId, getAgentUserId } from "./agent-user.js";
...
export async function findOrCreateCustomerByPhone(
  ...
  // Atribución al usuario/equipo "Agente IA" (fallback a null: crear el cliente
  // nunca debe fallar por la atribución).
  const createdBy = await getAgentUserId(tenant).catch(() => null);
  const teamId = createdBy ? await getAgentTeamId(tenant).catch(() => null) : null;
